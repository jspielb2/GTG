...
for ndx = 1:ninput
    if ~isempty(input{ndx})
        if input{ndx} == 1 && ndx ~= ninput
            progdata(ndx+1).starttime = clock;
        end
...
if progdata(1).fractiondone > 0
    for ndx = 1:ninput
        if ~isempty(input{ndx})
            runtime = etime(progdata(ndx).clock, progdata(ndx).starttime);
            timeleft = runtime / progdata(ndx).fractiondone - runtime;
            timeleftstr = sec2timestr(timeleft);
            titlebarstr = sprintf('%2d%%    %s remaining', ...
                floor(100*progdata(ndx).fractiondone), timeleftstr);
        end
    end
else
    titlebarstr = ' 0%';
