...
set(handles.Fullprop_listbox,'String',[{'None'};out.properties_calcd_fullmat]);
set(handles.Thrprop_listbox,'String',[{'None'};out.properties_calcd_thrmat]);
...
global out;
out.alpha = str2double(get(hObject,'String'));
...
global out;
out.num_perms = str2double(get(hObject,'String'));
...
out.HLA_reg_type = contents{get(hObject,'Value')};
...
global out;
contents = cellstr(get(hObject,'String'));
out.same_desmat = contents{get(hObject,'Value')};
if strcmp(out.same_desmat,'Yes')
    if isempty(out.denscalc_varmat)
        out.denscalc_varmat = ones(out.num_subs,1);
    end
    out.desmat   = [out.denscalc_varmat,out.denscalc_covarmat];
    out.IV_names = [out.denscalc_var_names,out.denscalc_covar_names];
    if rank([out.desmat,ones(size(out.desmat,1),1)]) == (size(out.desmat,2)+1) % If an intercept can be added without making the design matrix rank deficient
        out.desmat   = [out.desmat,ones(size(out.desmat,1),1)];
        out.IV_names = [out.IV_names,{'intercept'}];
    end
...
elseif strcmp(out.same_desmat,'No')
    out.desmat   = [];
    out.IV_names = [];
    set(handles.Contrasts_uitable,'Data',[]);
    set(handles.Contrasts_uitable,'ColumnName',[]);
...
out.desmat         = [];                                                                                                      % Set empty design matrix to start
...
for var = 1:length(var_selection)                                                                                              % For each IV selected
    out.IV_names{1,var} =  wksp_vars{var_selection(var)};
    out.desmat          = [out.desmat,evalin('base',wksp_vars{var_selection(var)})];                                                      % Get IV from workspace and add to design matrix
end
if rank([out.desmat,ones(size(out.desmat,1),1)]) == (size(out.desmat,2)+1) % If an intercept can be added without making the design matrix rank deficient
    out.desmat   = [out.desmat,ones(size(out.desmat,1),1)];
    out.IV_names = [out.IV_names,{'intercept'}];
...
global out;
contents          = cellstr(get(hObject,'String'));
out.Contrast_or_F = contents{get(hObject,'Value')};
...
global out;
out.num_contrasts = str2double(get(hObject,'String'));
def_contrasts     = zeros(out.num_contrasts,size(out.desmat,2));
set(handles.Contrasts_uitable,'Data',[]);
set(handles.Contrasts_uitable,'ColumnName',[]);
set(handles.Contrasts_uitable,'Data',def_contrasts);
set(handles.Contrasts_uitable,'Columneditable',true(1,size(out.desmat,2)));
set(handles.Contrasts_uitable,'ColumnName',out.IV_names);
...
global out
out.contrasts = get(hObject,'Data');
...
global out;
out.test_all_nodes = get(hObject,'Value');

if out.test_all_nodes == 1
    out.node_mask = ones(out.nROI,1);
    set(handles.Nodemask_edit,'enable','off');
    set(handles.Nodeselect_pushbutton,'enable','off');
elseif out.test_all_nodes == 0
...
global out;
temp          = get(hObject,'String');
out.node_mask = evalin('base',temp);
out.test_all_nodes = 0;
...
global out;
[selection]              = listdlg('PromptString',{'Select ALL nodes to test:',''},'SelectionMode','multiple','ListString',char(out.ROI_labels),'initialvalue',1);
out.node_mask            = zeros(out.nROI,1);
out.node_mask(selection) = 1;
out.test_all_nodes       = 0;
...
global out;
out.test_all_edges = get(hObject,'Value');

if out.test_all_edges == 1
    out.edge_mask = triu(ones(out.nROI,out.nROI),1);
...
global out;
temp          = get(hObject,'String');
out.edge_mask = evalin('base',temp);
...
[node_selection] = listdlg('PromptString',{'Select ALL nodes that are linked by your edges of interest:',''},'SelectionMode','multiple','ListString',char(out.ROI_labels),'initialvalue',1);
nodenames = {out.ROI_labels{node_selection}}; %#ok<CCAT1>
if size(nodenames,1) < size(nodenames,2)
    nodenames = nodenames';
end

out.edge_mask = zeros(out.nROI,out.nROI);
for curr_edge = 1:num_edges
    ...
    out.edge_mask(node_selection(edge_selection(1)),node_selection(edge_selection(2)),:) = 1;
end
out.test_all_edges = 0;
...
global out;
contents                      = cellstr(get(hObject,'String'));
out.properties_tested_fullmat = contents(get(hObject,'Value'));
...
global out;
contents                     = cellstr(get(hObject,'String'));
out.properties_tested_thrmat = contents(get(hObject,'Value'));
...
if out.test_props_thrmat.clust_coef == 1 || out.test_props_thrmat.deg == 1 || out.test_props_fullmat.div_coef == 1 || out.test_props_fullmat.eigvec_cent == 1 || out.test_props_thrmat.eigvec_cent == 1 || out.test_props_thrmat.kcore_cent == 1 || out.test_props_thrmat.loc_eff == 1 || out.test_props_fullmat.node_bet_cent == 1 || out.test_props_thrmat.node_bet_cent == 1 || out.test_props_fullmat.strength == 1 || out.test_props_fullmat.pagerank_cent == 1 || out.test_props_thrmat.pagerank_cent == 1 || out.test_props_fullmat.part_coef == 1 || out.test_props_thrmat.sub_cent == 1 || out.test_props_fullmat.mod_deg_z == 1
...
if out.test_props_fullmat.edge_bet_cent == 1 || out.test_props_thrmat.edge_bet_cent == 1 || out.test_props_thrmat.match == 1
...
for con = 1:size(out.contrasts,1)
    ...
    tot_props_to_calc = out.test_props_fullmat.assort+out.test_props_fullmat.cpl+ ...
        out.test_props_fullmat.div_coef+out.test_props_fullmat.edge_bet_cent+ ...
        out.test_props_fullmat.eigvec_cent+out.test_props_fullmat.glob_eff+ ...
        out.test_props_fullmat.node_bet_cent+out.test_props_fullmat.strength+ ...
        out.test_props_fullmat.pagerank_cent+out.test_props_fullmat.part_coef+ ...
        out.test_props_fullmat.mod_deg_z+out.test_props_thrmat.assort+ ...
        out.test_props_thrmat.cpl+out.test_props_thrmat.clust_coef+ ...
        out.test_props_thrmat.deg+out.test_props_thrmat.dens+ ...
        out.test_props_thrmat.edge_bet_cent+out.test_props_thrmat.eigvec_cent+ ...
        out.test_props_thrmat.glob_eff+out.test_props_thrmat.kcore_cent+ ...
        out.test_props_thrmat.loc_eff+out.test_props_thrmat.match+ ...
        out.test_props_thrmat.node_bet_cent+out.test_props_thrmat.pagerank_cent+ ...
        out.test_props_thrmat.rich_club+out.test_props_thrmat.small_world+ ...
        out.test_props_thrmat.sub_cent+out.test_props_thrmat.trans;
    curr_props_calcd = 0;
    ...
    %%%% Properties for fully connected networks only
    ...
    if out.test_props_fullmat.mod_deg_z == 1
        for curr_ROI = out.nROI:-1:1
            if out.node_mask(curr_ROI) == 1
                [out.full.mod_deg_z.beta(con,curr_ROI,:), ...
                    out.full.mod_deg_z.test_stat(con,curr_ROI,:), ...
                    out.full.mod_deg_z.crit_val(con,curr_ROI,:), ...
                    out.full.mod_deg_z.p_1tail(con,curr_ROI,:), ...
                    out.full.mod_deg_z.p_2tail(con,curr_ROI,:), ...
                    out.full.mod_deg_z.hadNaN(con,curr_ROI,:), ...
                    out.full.mod_deg_z.hadimag(con,curr_ROI,:), ...
                    out.full.mod_deg_z.hadInf(con,curr_ROI,:)] = ...
                    GTG_GLM(squeeze(out.fullmat_graph_meas.mod_deg_z(:,curr_ROI,:)), ...
                    curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
            else
                out.full.mod_deg_z.hadNaN(con,curr_ROI,:)    = NaN;
                out.full.mod_deg_z.hadimag(con,curr_ROI,:)   = NaN;
                out.full.mod_deg_z.hadInf(con,curr_ROI,:)    = NaN;
                out.full.mod_deg_z.beta(con,curr_ROI,:)      = NaNout;
                out.full.mod_deg_z.test_stat(con,curr_ROI,:) = NaNout;
                out.full.mod_deg_z.crit_val(con,curr_ROI,:)  = NaNout;
                out.full.mod_deg_z.p_1tail(con,curr_ROI,:)   = NaNout;
                out.full.mod_deg_z.p_2tail(con,curr_ROI,:)   = NaNout;
            end
        end
    ...
    %%%% Properties for thresholded networks only
    ...
    if out.test_props_thrmat.rich_club == 1
        for curr_size = out.max_club_size_pos:-1:1
    ...
    %%%% Properties for both fully connected and thresholded networks
    if out.test_props_fullmat.assort == 1
        [out.full.assort.beta(con,:), ...
            out.full.assort.test_stat(con,:), ...
            out.full.assort.crit_val(con,:), ...
            out.full.assort.p_1tail(con,:), ...
            out.full.assort.p_2tail(con,:), ...
            out.full.assort.hadNaN(con,:), ...
            out.full.assort.hadimag(con,:), ...
            out.full.assort.hadInf(con,:)] = ...
            GTG_GLM(out.fullmat_graph_meas.assort, ...
            curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
    ...
    if out.test_props_fullmat.cpl == 1
        [out.full.cpl.beta(con,:), ...
            out.full.cpl.test_stat(con,:), ...
            out.full.cpl.crit_val(con,:), ...
            out.full.cpl.p_1tail(con,:), ...
            out.full.cpl.p_2tail(con,:), ...
            out.full.cpl.hadNaN(con,:), ...
            out.full.cpl.hadimag(con,:), ...
            out.full.cpl.hadInf(con,:)] = ...
            GTG_GLM(out.fullmat_graph_meas.cpl, ...
            curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
    ...
    if out.test_props_fullmat.eigvec_cent == 1
        for curr_ROI = out.nROI:-1:1
            if out.node_mask(curr_ROI) == 1 && any(any(squeeze(out.fullmat_graph_meas.eigvec_cent(:,curr_ROI,:))))
                [out.full.eigvec_cent.beta(con,curr_ROI,:), ...
                    out.full.eigvec_cent.test_stat(con,curr_ROI,:), ...
                    out.full.eigvec_cent.crit_val(con,curr_ROI,:), ...
                    out.full.eigvec_cent.p_1tail(con,curr_ROI,:), ...
                    out.full.eigvec_cent.p_2tail(con,curr_ROI,:), ...
                    out.full.eigvec_cent.hadNaN(con,curr_ROI,:), ...
                    out.full.eigvec_cent.hadimag(con,curr_ROI,:), ...
                    out.full.eigvec_cent.hadInf(con,curr_ROI,:)] = ...
                    GTG_GLM(squeeze(out.fullmat_graph_meas.eigvec_cent(:,curr_ROI,:)), ...
                    curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
            else
                out.full.eigvec_cent.hadNaN(con,curr_ROI,:)    = NaN;
                out.full.eigvec_cent.hadimag(con,curr_ROI,:)   = NaN;
                out.full.eigvec_cent.hadInf(con,curr_ROI,:)    = NaN;
                out.full.eigvec_cent.beta(con,curr_ROI,:)      = NaNout;
                out.full.eigvec_cent.test_stat(con,curr_ROI,:) = NaNout;
                out.full.eigvec_cent.crit_val(con,curr_ROI,:)  = NaNout;
                out.full.eigvec_cent.p_1tail(con,curr_ROI,:)   = NaNout;
                out.full.eigvec_cent.p_2tail(con,curr_ROI,:)   = NaNout;
            end
        end
        ...
        for curr_row_ROI = out.nROI:-1:1
            for curr_col_ROI = out.nROI:-1:1
                if out.edge_mask(curr_row_ROI,curr_col_ROI) == 1 && any(any(squeeze(out.fullmat_graph_meas.edge_bet_cent(:,curr_row_ROI,curr_col_ROI,:))))
                    [out.full.edge_bet_cent.beta(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.test_stat(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.crit_val(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.p_1tail(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.p_2tail(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.hadNaN(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.hadimag(con,curr_row_ROI,curr_col_ROI,:), ...
                        out.full.edge_bet_cent.hadInf(con,curr_row_ROI,curr_col_ROI,:)] = ...
                        GTG_GLM(squeeze(out.fullmat_graph_meas.edge_bet_cent(:,curr_row_ROI,curr_col_ROI,:)), ...
                        curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
                else
                    out.full.edge_bet_cent.hadNaN(con,curr_row_ROI,curr_col_ROI,:)    = NaN;
                    out.full.edge_bet_cent.hadimag(con,curr_row_ROI,curr_col_ROI,:)   = NaN;
                    out.full.edge_bet_cent.hadInf(con,curr_row_ROI,curr_col_ROI,:)    = NaN;
                    out.full.edge_bet_cent.beta(con,curr_row_ROI,curr_col_ROI,:)      = NaNout;
                    out.full.edge_bet_cent.test_stat(con,curr_row_ROI,curr_col_ROI,:) = NaNout;
                    out.full.edge_bet_cent.crit_val(con,curr_row_ROI,curr_col_ROI,:)  = NaNout;
                    out.full.edge_bet_cent.p_1tail(con,curr_row_ROI,curr_col_ROI,:)   = NaNout;
                    out.full.edge_bet_cent.p_2tail(con,curr_row_ROI,curr_col_ROI,:)   = NaNout;
                end
            end
        end
    ...
    if out.test_props_fullmat.glob_eff == 1
        [out.full.glob_eff.beta(con,:), ...
            out.full.glob_eff.test_stat(con,:), ...
            out.full.glob_eff.crit_val(con,:), ...
            out.full.glob_eff.p_1tail(con,:), ...
            out.full.glob_eff.p_2tail(con,:), ...
            out.full.glob_eff.hadNaN(con,:), ...
            out.full.glob_eff.hadimag(con,:), ...
            out.full.glob_eff.hadInf(con,:)] = ...
            GTG_GLM(out.fullmat_graph_meas.glob_eff, ...
            curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
    ...
    if out.test_props_fullmat.node_bet_cent == 1
        for curr_ROI = out.nROI:-1:1
            if out.node_mask(curr_ROI) == 1
                [out.full.node_bet_cent.beta(con,curr_ROI,:), ...
                    out.full.node_bet_cent.test_stat(con,curr_ROI,:), ...
                    out.full.node_bet_cent.crit_val(con,curr_ROI,:), ...
                    out.full.node_bet_cent.p_1tail(con,curr_ROI,:), ...
                    out.full.node_bet_cent.p_2tail(con,curr_ROI,:), ...
                    out.full.node_bet_cent.hadNaN(con,curr_ROI,:), ...
                    out.full.node_bet_cent.hadimag(con,curr_ROI,:), ...
                    out.full.node_bet_cent.hadInf(con,curr_ROI,:)] = ...
                    GTG_GLM(squeeze(out.fullmat_graph_meas.node_bet_cent(:,curr_ROI,:)), ...
                    curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
            else
                out.full.node_bet_cent.hadNaN(con,curr_ROI,:)    = NaN;
                out.full.node_bet_cent.hadimag(con,curr_ROI,:)   = NaN;
                out.full.node_bet_cent.hadInf(con,curr_ROI,:)    = NaN;
                out.full.node_bet_cent.beta(con,curr_ROI,:)      = NaNout;
                out.full.node_bet_cent.test_stat(con,curr_ROI,:) = NaNout;
                out.full.node_bet_cent.crit_val(con,curr_ROI,:)  = NaNout;
                out.full.node_bet_cent.p_1tail(con,curr_ROI,:)   = NaNout;
                out.full.node_bet_cent.p_2tail(con,curr_ROI,:)   = NaNout;
            end
        end
    ...
    if out.test_props_fullmat.pagerank_cent == 1
        for curr_ROI = out.nROI:-1:1
            if out.node_mask(curr_ROI) == 1 && any(any(squeeze(out.fullmat_graph_meas.pagerank_cent(:,curr_ROI,:))))
                [out.full.pagerank_cent.beta(con,curr_ROI,:), ...
                    out.full.pagerank_cent.test_stat(con,curr_ROI,:), ...
                    out.full.pagerank_cent.crit_val(con,curr_ROI,:), ...
                    out.full.pagerank_cent.p_1tail(con,curr_ROI,:), ...
                    out.full.pagerank_cent.p_2tail(con,curr_ROI,:), ...
                    out.full.pagerank_cent.hadNaN(con,curr_ROI,:), ...
                    out.full.pagerank_cent.hadimag(con,curr_ROI,:), ...
                    out.full.pagerank_cent.hadInf(con,curr_ROI,:)] = ...
                    GTG_GLM(squeeze(out.fullmat_graph_meas.pagerank_cent(:,curr_ROI,:)), ...
                    curr_full_desmat,currcovars_desmat,out.Contrast_or_F,perms,out.HLA_reg_type,use_parfor,within_perms,out.alpha);
            else
                out.full.pagerank_cent.hadNaN(con,curr_ROI,:)    = NaN;
                out.full.pagerank_cent.hadimag(con,curr_ROI,:)   = NaN;
                out.full.pagerank_cent.hadInf(con,curr_ROI,:)    = NaN;
                out.full.pagerank_cent.beta(con,curr_ROI,:)      = NaNout;
                out.full.pagerank_cent.test_stat(con,curr_ROI,:) = NaNout;
                out.full.pagerank_cent.crit_val(con,curr_ROI,:)  = NaNout;
                out.full.pagerank_cent.p_1tail(con,curr_ROI,:)   = NaNout;
                out.full.pagerank_cent.p_2tail(con,curr_ROI,:)   = NaNout;
            end
        end
    ...
    for lev = 1:lev_vals
        ...
        if out.test_props_thrmat.assort == 1
            if out.full.assort.p_2tail(con,lev) <= out.alpha && ~isnan(out.full.assort.beta(con,lev)) && out.full.assort.beta(con,lev) ~= 0
                out.sig_find(con).assort_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).assort_full{2,lev} = {'beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).assort_full{2,lev} = [out.sig_find(con).assort_full{2,lev};num2cell([out.full.assort.beta(con,lev);out.full.assort.test_stat(con,lev);out.full.assort.crit_val(con,lev);out.full.assort.p_2tail(con,lev);out.full.assort.hadNaN(con);out.full.assort.hadimag(con);out.full.assort.hadInf(con)]')];
                fprintf(sigeffects_fid,'Assortativity (full matrices):\n\n');
                fprintf(sigeffects_fid,'beta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.full.assort.beta(con,lev),out.full.assort.test_stat(con,lev),out.full.assort.crit_val(con,lev),out.full.assort.p_2tail(con,lev),out.full.assort.hadNaN(con),out.full.assort.hadimag(con),out.full.assort.hadInf(con));
            end
        ...
        if out.test_props_fullmat.cpl == 1
            if out.full.cpl.p_2tail(con,lev) <= out.alpha && ~isnan(out.full.cpl.beta(con,lev)) && out.full.cpl.beta(con,lev) ~= 0
                out.sig_find(con).cpl_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).cpl_full{2,lev} = {'beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).cpl_full{2,lev} = [out.sig_find(con).cpl_full{2,lev};num2cell([out.full.cpl.beta(con,lev);out.full.cpl.test_stat(con,lev);out.full.cpl.crit_val(con,lev);out.full.cpl.p_2tail(con,lev);out.full.cpl.hadNaN(con);out.full.cpl.hadimag(con);out.full.cpl.hadInf(con)]')];
                
                fprintf(sigeffects_fid,'Characteristic Path Length (full matrices):\n\n');
                fprintf(sigeffects_fid,'beta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.full.cpl.beta(con,lev),out.full.cpl.test_stat(con,lev),out.full.cpl.crit_val(con,lev),out.full.cpl.p_2tail(con,lev),out.full.cpl.hadNaN(con),out.full.cpl.hadimag(con),out.full.cpl.hadInf(con));
            end
        end
        if out.test_props_thrmat.cpl == 1
        ...
        if out.test_props_thrmat.clust_coef == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.clust_coef_pos.beta(con,ind,lev),out.thrAUC.clust_coef_pos.test_stat(con,ind,lev),out.thrAUC.clust_coef_pos.crit_val(con,ind,lev),out.thrAUC.clust_coef_pos.p_2tail(con,ind,lev),out.thrAUC.clust_coef_pos.hadNaN(con,ind),out.thrAUC.clust_coef_pos.hadimag(con,ind),out.thrAUC.clust_coef_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.clust_coef_neg.beta(con,ind,lev),out.thrAUC.clust_coef_neg.test_stat(con,ind,lev),out.thrAUC.clust_coef_neg.crit_val(con,ind,lev),out.thrAUC.clust_coef_neg.p_2tail(con,ind,lev),out.thrAUC.clust_coef_neg.hadNaN(con,ind),out.thrAUC.clust_coef_neg.hadimag(con,ind),out.thrAUC.clust_coef_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.clust_coef_pos_nodiscon.beta(con,ind,lev),out.thrAUC.clust_coef_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.clust_coef_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.clust_coef_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.clust_coef_pos_nodiscon.hadNaN(con,ind),out.thrAUC.clust_coef_pos_nodiscon.hadimag(con,ind),out.thrAUC.clust_coef_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.clust_coef_neg_nodiscon.beta(con,ind,lev),out.thrAUC.clust_coef_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.clust_coef_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.clust_coef_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.clust_coef_neg_nodiscon.hadNaN(con,ind),out.thrAUC.clust_coef_neg_nodiscon.hadimag(con,ind),out.thrAUC.clust_coef_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_thrmat.deg == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.deg_pos.beta(con,ind,lev),out.thrAUC.deg_pos.test_stat(con,ind,lev),out.thrAUC.deg_pos.crit_val(con,ind,lev),out.thrAUC.deg_pos.p_2tail(con,ind,lev),out.thrAUC.deg_pos.hadNaN(con,ind),out.thrAUC.deg_pos.hadimag(con,ind),out.thrAUC.deg_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.deg_neg.beta(con,ind,lev),out.thrAUC.deg_neg.test_stat(con,ind,lev),out.thrAUC.deg_neg.crit_val(con,ind,lev),out.thrAUC.deg_neg.p_2tail(con,ind,lev),out.thrAUC.deg_neg.hadNaN(con,ind),out.thrAUC.deg_neg.hadimag(con,ind),out.thrAUC.deg_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.deg_pos_nodiscon.beta(con,ind,lev),out.thrAUC.deg_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.deg_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.deg_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.deg_pos_nodiscon.hadNaN(con,ind),out.thrAUC.deg_pos_nodiscon.hadimag(con,ind),out.thrAUC.deg_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.deg_neg_nodiscon.beta(con,ind,lev),out.thrAUC.deg_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.deg_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.deg_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.deg_neg_nodiscon.hadNaN(con,ind),out.thrAUC.deg_neg_nodiscon.hadimag(con,ind),out.thrAUC.deg_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.div_coef == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.div_coef_neg.beta(con,ind,lev),out.full.div_coef_neg.test_stat(con,ind,lev),out.full.div_coef_neg.crit_val(con,ind,lev),out.full.div_coef_neg.p_2tail(con,ind,lev),out.full.div_coef_neg.hadNaN(con,ind),out.full.div_coef_neg.hadimag(con,ind),out.full.div_coef_neg.hadInf(con,ind));
            end
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.div_coef_pos.beta(con,ind,lev),out.full.div_coef_pos.test_stat(con,ind,lev),out.full.div_coef_pos.crit_val(con,ind,lev),out.full.div_coef_pos.p_2tail(con,ind,lev),out.full.div_coef_pos.hadNaN(con,ind),out.full.div_coef_pos.hadimag(con,ind),out.full.div_coef_pos.hadInf(con,ind));
            end
        end
        
        if out.test_props_fullmat.edge_bet_cent == 1
            ind                          = logical((squeeze(out.full.edge_bet_cent.p_2tail(con,:,:,lev)) <= out.alpha) & ~isnan(squeeze(out.full.edge_bet_cent.beta(con,:,:,lev))) & squeeze(out.full.edge_bet_cent.beta(con,:,:,lev) ~= 0));
            ind(logical(eye(size(ind)))) = 0;
            [xind,yind]                  = find(ind == 1);
            if any(any(ind))
                out.sig_find(con).edge_bet_cent_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).edge_bet_cent_full{2,lev} = {'node_1','node_2','beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                tempbeta                                  = squeeze(out.full.edge_bet_cent.beta(con,:,:,lev));
                tempteststat                              = squeeze(out.full.edge_bet_cent.test_stat(con,:,:,lev));
                tempcritval                               = squeeze(out.full.edge_bet_cent.crit_val(con,:,:,lev));
                tempp                                     = squeeze(out.full.edge_bet_cent.p_2tail(con,:,:,lev));
                out.sig_find(con).edge_bet_cent_full{2,lev} = [out.sig_find(con).edge_bet_cent_full{2,lev};[out.ROI_labels(xind),out.ROI_labels(yind),num2cell([tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.full.edge_bet_cent.hadNaN(con,ind)',out.full.edge_bet_cent.hadimag(con,ind)',out.full.edge_bet_cent.hadInf(con,ind)'])]];
                
                fprintf(sigeffects_fid,'Edge Betweeness Centrality (full matrices):\n\n');
                fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.full.edge_bet_cent.hadNaN(con,xind,yind),out.full.edge_bet_cent.hadimag(con,xind,yind),out.full.edge_bet_cent.hadInf(con,xind,yind));
            end
        end
        if out.test_props_thrmat.edge_bet_cent == 1
            ...
            if any(any(ind))
                ...
                fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.edge_bet_cent_pos.hadNaN(con,xind,yind),out.thrAUC.edge_bet_cent_pos.hadimag(con,xind,yind),out.thrAUC.edge_bet_cent_pos.hadInf(con,xind,yind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(any(ind))
                    ...
                    fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.edge_bet_cent_neg.hadNaN(con,xind,yind),out.thrAUC.edge_bet_cent_neg.hadimag(con,xind,yind),out.thrAUC.edge_bet_cent_neg.hadInf(con,xind,yind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(any(ind))
                    ...
                    fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.edge_bet_cent_pos_nodiscon.hadNaN(con,xind,yind),out.thrAUC.edge_bet_cent_pos_nodiscon.hadimag(con,xind,yind),out.thrAUC.edge_bet_cent_pos_nodiscon.hadInf(con,xind,yind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(any(ind))
                        ...
                        fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.edge_bet_cent_neg_nodiscon.hadNaN(con,xind,yind),out.thrAUC.edge_bet_cent_neg_nodiscon.hadimag(con,xind,yind),out.thrAUC.edge_bet_cent_neg_nodiscon.hadInf(con,xind,yind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.eigvec_cent == 1
            ind = logical(out.full.eigvec_cent.p_2tail(con,:,lev) <= out.alpha & ~isnan(out.full.eigvec_cent.beta(con,:,lev)) & out.full.eigvec_cent.beta(con,:,lev) ~= 0);
            if any(ind)
                out.sig_find(con).eigvec_cent_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).eigvec_cent_full{2,lev} = {'node','beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).eigvec_cent_full{2,lev} = [out.sig_find(con).eigvec_cent_full{2,lev};[out.ROI_labels(ind),num2cell([out.full.eigvec_cent.beta(con,ind,lev);out.full.eigvec_cent.test_stat(con,ind,lev);out.full.eigvec_cent.crit_val(con,ind,lev);out.full.eigvec_cent.p_2tail(con,ind,lev);out.full.eigvec_cent.hadNaN(con,ind);out.full.eigvec_cent.hadimag(con,ind);out.full.eigvec_cent.hadInf(con,ind)]')]];
                
                fprintf(sigeffects_fid,'Eigenvector Centrality (full matrices):\n\n');
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.eigvec_cent.beta(con,ind,lev),out.full.eigvec_cent.test_stat(con,ind,lev),out.full.eigvec_cent.crit_val(con,ind,lev),out.full.eigvec_cent.p_2tail(con,ind,lev),out.full.eigvec_cent.hadNaN(con,ind),out.full.eigvec_cent.hadimag(con,ind),out.full.eigvec_cent.hadInf(con,ind));
            end
        end
        if out.test_props_thrmat.eigvec_cent == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.eigvec_cent_pos.beta(con,ind,lev),out.thrAUC.eigvec_cent_pos.test_stat(con,ind,lev),out.thrAUC.eigvec_cent_pos.crit_val(con,ind,lev),out.thrAUC.eigvec_cent_pos.p_2tail(con,ind,lev),out.thrAUC.eigvec_cent_pos.hadNaN(con,ind),out.thrAUC.eigvec_cent_pos.hadimag(con,ind),out.thrAUC.eigvec_cent_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.eigvec_cent_neg.beta(con,ind,lev),out.thrAUC.eigvec_cent_neg.test_stat(con,ind,lev),out.thrAUC.eigvec_cent_neg.crit_val(con,ind,lev),out.thrAUC.eigvec_cent_neg.p_2tail(con,ind,lev),out.thrAUC.eigvec_cent_neg.hadNaN(con,ind),out.thrAUC.eigvec_cent_neg.hadimag(con,ind),out.thrAUC.eigvec_cent_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.eigvec_cent_pos_nodiscon.beta(con,ind,lev),out.thrAUC.eigvec_cent_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.eigvec_cent_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.eigvec_cent_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.eigvec_cent_pos_nodiscon.hadNaN(con,ind),out.thrAUC.eigvec_cent_pos_nodiscon.hadimag(con,ind),out.thrAUC.eigvec_cent_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.eigvec_cent_neg_nodiscon.beta(con,ind,lev),out.thrAUC.eigvec_cent_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.eigvec_cent_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.eigvec_cent_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.eigvec_cent_neg_nodiscon.hadNaN(con,ind),out.thrAUC.eigvec_cent_neg_nodiscon.hadimag(con,ind),out.thrAUC.eigvec_cent_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.glob_eff == 1
            if out.full.glob_eff.p_2tail(con,lev) <= out.alpha && ~isnan(out.full.glob_eff.beta(con,lev)) && out.full.glob_eff.beta(con,lev) ~= 0
                out.sig_find(con).glob_eff_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).glob_eff_full{2,lev} = {'beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).glob_eff_full{2,lev} = [out.sig_find(con).glob_eff_full{2,lev};num2cell([out.full.glob_eff.beta(con,lev);out.full.glob_eff.test_stat(con,lev);out.full.glob_eff.crit_val(con,lev);out.full.glob_eff.p_2tail(con,lev);out.full.glob_eff.hadNaN(con);out.full.glob_eff.hadimag(con);out.full.glob_eff.hadInf(con)]')];
                
                fprintf(sigeffects_fid,'Global Efficiency (full matrices):\n\n');
                fprintf(sigeffects_fid,'beta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.full.glob_eff.beta(con,lev),out.full.glob_eff.test_stat(con,lev),out.full.glob_eff.crit_val(con,lev),out.full.glob_eff.p_2tail(con,lev),out.full.glob_eff.hadNaN(con),out.full.glob_eff.hadimag(con),out.full.glob_eff.hadInf(con));
            end
        end
        if out.test_props_thrmat.glob_eff == 1
        ...
        if out.test_props_thrmat.kcore_cent == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.kcore_cent_pos.beta(con,ind,lev),out.thrAUC.kcore_cent_pos.test_stat(con,ind,lev),out.thrAUC.kcore_cent_pos.crit_val(con,ind,lev),out.thrAUC.kcore_cent_pos.p_2tail(con,ind,lev),out.thrAUC.kcore_cent_pos.hadNaN(con,ind),out.thrAUC.kcore_cent_pos.hadimag(con,ind),out.thrAUC.kcore_cent_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.kcore_cent_neg.beta(con,ind,lev),out.thrAUC.kcore_cent_neg.test_stat(con,ind,lev),out.thrAUC.kcore_cent_neg.crit_val(con,ind,lev),out.thrAUC.kcore_cent_neg.p_2tail(con,ind,lev),out.thrAUC.kcore_cent_neg.hadNaN(con,ind),out.thrAUC.kcore_cent_neg.hadimag(con,ind),out.thrAUC.kcore_cent_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.kcore_cent_pos_nodiscon.beta(con,ind,lev),out.thrAUC.kcore_cent_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.kcore_cent_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.kcore_cent_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.kcore_cent_pos_nodiscon.hadNaN(con,ind),out.thrAUC.kcore_cent_pos_nodiscon.hadimag(con,ind),out.thrAUC.kcore_cent_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.kcore_cent_neg_nodiscon.beta(con,ind,lev),out.thrAUC.kcore_cent_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.kcore_cent_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.kcore_cent_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.kcore_cent_neg_nodiscon.hadNaN(con,ind),out.thrAUC.kcore_cent_neg_nodiscon.hadimag(con,ind),out.thrAUC.kcore_cent_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_thrmat.loc_eff == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.loc_eff_pos.beta(con,ind,lev),out.thrAUC.loc_eff_pos.test_stat(con,ind,lev),out.thrAUC.loc_eff_pos.crit_val(con,ind,lev),out.thrAUC.loc_eff_pos.p_2tail(con,ind,lev),out.thrAUC.loc_eff_pos.hadNaN(con,ind),out.thrAUC.loc_eff_pos.hadimag(con,ind),out.thrAUC.loc_eff_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.loc_eff_neg.beta(con,ind,lev),out.thrAUC.loc_eff_neg.test_stat(con,ind,lev),out.thrAUC.loc_eff_neg.crit_val(con,ind,lev),out.thrAUC.loc_eff_neg.p_2tail(con,ind,lev),out.thrAUC.loc_eff_neg.hadNaN(con,ind),out.thrAUC.loc_eff_neg.hadimag(con,ind),out.thrAUC.loc_eff_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.loc_eff_pos_nodiscon.beta(con,ind,lev),out.thrAUC.loc_eff_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.loc_eff_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.loc_eff_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.loc_eff_pos_nodiscon.hadNaN(con,ind),out.thrAUC.loc_eff_pos_nodiscon.hadimag(con,ind),out.thrAUC.loc_eff_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.loc_eff_neg_nodiscon.beta(con,ind,lev),out.thrAUC.loc_eff_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.loc_eff_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.loc_eff_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.loc_eff_neg_nodiscon.hadNaN(con,ind),out.thrAUC.loc_eff_neg_nodiscon.hadimag(con,ind),out.thrAUC.loc_eff_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_thrmat.match == 1
            ...
            if any(any(ind))
                ...
                fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.match_pos.hadNaN(con,xind,yind),out.thrAUC.match_pos.hadimag(con,xind,yind),out.thrAUC.match_pos.hadInf(con,xind,yind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(any(ind))
                    ...
                    fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.match_neg.hadNaN(con,xind,yind),out.thrAUC.match_neg.hadimag(con,xind,yind),out.thrAUC.match_neg.hadInf(con,xind,yind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(any(ind))
                    ...
                    fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.match_pos_nodiscon.hadNaN(con,xind,yind),out.thrAUC.match_pos_nodiscon.hadimag(con,xind,yind),out.thrAUC.match_pos_nodiscon.hadInf(con,xind,yind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(any(ind))
                        ...
                        fprintf(sigeffects_fid,'node_1\tnode_2\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels(xind),out.ROI_labels(yind),tempbeta(ind),tempteststat(ind),tempcritval(ind),tempp(ind),out.thrAUC.match_neg_nodiscon.hadNaN(con,xind,yind),out.thrAUC.match_neg_nodiscon.hadimag(con,xind,yind),out.thrAUC.match_neg_nodiscon.hadInf(con,xind,yind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.node_bet_cent == 1
            ind = logical(out.full.node_bet_cent.p_2tail(con,:,lev) <= out.alpha & ~isnan(out.full.node_bet_cent.beta(con,:,lev)) & out.full.node_bet_cent.beta(con,:,lev) ~= 0);
            if any(ind)
                out.sig_find(con).node_bet_cent_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).node_bet_cent_full{2,lev} = {'node','beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).node_bet_cent_full{2,lev} = [out.sig_find(con).node_bet_cent_full{2,lev};[out.ROI_labels(ind),num2cell([out.full.node_bet_cent.beta(con,ind,lev);out.full.node_bet_cent.test_stat(con,ind,lev);out.full.node_bet_cent.crit_val(con,ind,lev);out.full.node_bet_cent.p_2tail(con,ind,lev);out.full.node_bet_cent.hadNaN(con,ind);out.full.node_bet_cent.hadimag(con,ind);out.full.node_bet_cent.hadInf(con,ind)]')]];
                
                fprintf(sigeffects_fid,'Node Betweeness Centrality (full matrices):\n\n');
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.node_bet_cent.beta(con,ind,lev),out.full.node_bet_cent.test_stat(con,ind,lev),out.full.node_bet_cent.crit_val(con,ind,lev),out.full.node_bet_cent.p_2tail(con,ind,lev),out.full.node_bet_cent.hadNaN(con,ind),out.full.node_bet_cent.hadimag(con,ind),out.full.node_bet_cent.hadInf(con,ind));
            end
        end
        if out.test_props_thrmat.node_bet_cent == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.node_bet_cent_pos.beta(con,ind,lev),out.thrAUC.node_bet_cent_pos.test_stat(con,ind,lev),out.thrAUC.node_bet_cent_pos.crit_val(con,ind,lev),out.thrAUC.node_bet_cent_pos.p_2tail(con,ind,lev),out.thrAUC.node_bet_cent_pos.hadNaN(con,ind),out.thrAUC.node_bet_cent_pos.hadimag(con,ind),out.thrAUC.node_bet_cent_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.node_bet_cent_neg.beta(con,ind,lev),out.thrAUC.node_bet_cent_neg.test_stat(con,ind,lev),out.thrAUC.node_bet_cent_neg.crit_val(con,ind,lev),out.thrAUC.node_bet_cent_neg.p_2tail(con,ind,lev),out.thrAUC.node_bet_cent_neg.hadNaN(con,ind),out.thrAUC.node_bet_cent_neg.hadimag(con,ind),out.thrAUC.node_bet_cent_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.node_bet_cent_pos_nodiscon.beta(con,ind,lev),out.thrAUC.node_bet_cent_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.node_bet_cent_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.node_bet_cent_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.node_bet_cent_pos_nodiscon.hadNaN(con,ind),out.thrAUC.node_bet_cent_pos_nodiscon.hadimag(con,ind),out.thrAUC.node_bet_cent_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.node_bet_cent_neg_nodiscon.beta(con,ind,lev),out.thrAUC.node_bet_cent_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.node_bet_cent_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.node_bet_cent_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.node_bet_cent_neg_nodiscon.hadNaN(con,ind),out.thrAUC.node_bet_cent_neg_nodiscon.hadimag(con,ind),out.thrAUC.node_bet_cent_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.pagerank_cent == 1
            ind = logical(out.full.pagerank_cent.p_2tail(con,:,lev) <= out.alpha & ~isnan(out.full.pagerank_cent.beta(con,:,lev)) & out.full.pagerank_cent.beta(con,:,lev) ~= 0);
            if any(ind)
                out.sig_find(con).pagerank_cent_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).pagerank_cent_full{2,lev} = {'node','beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).pagerank_cent_full{2,lev} = [out.sig_find(con).pagerank_cent_full{2,lev};[out.ROI_labels(ind),num2cell([out.full.pagerank_cent.beta(con,ind,lev);out.full.pagerank_cent.test_stat(con,ind,lev);out.full.pagerank_cent.crit_val(con,ind,lev);out.full.pagerank_cent.p_2tail(con,ind,lev);out.full.pagerank_cent.hadNaN(con,ind);out.full.pagerank_cent.hadimag(con,ind);out.full.pagerank_cent.hadInf(con,ind)]')]];
                
                fprintf(sigeffects_fid,'Pagerank Centrality (full matrices):\n\n');
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.pagerank_cent.beta(con,ind,lev),out.full.pagerank_cent.test_stat(con,ind,lev),out.full.pagerank_cent.crit_val(con,ind,lev),out.full.pagerank_cent.p_2tail(con,ind,lev),out.full.pagerank_cent.hadNaN(con,ind),out.full.pagerank_cent.hadimag(con,ind),out.full.pagerank_cent.hadInf(con,ind));
            end
        end
        if out.test_props_thrmat.pagerank_cent == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.pagerank_cent_pos.beta(con,ind,lev),out.thrAUC.pagerank_cent_pos.test_stat(con,ind,lev),out.thrAUC.pagerank_cent_pos.crit_val(con,ind,lev),out.thrAUC.pagerank_cent_pos.p_2tail(con,ind,lev),out.thrAUC.pagerank_cent_pos.hadNaN(con,ind),out.thrAUC.pagerank_cent_pos.hadimag(con,ind),out.thrAUC.pagerank_cent_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.pagerank_cent_neg.beta(con,ind,lev),out.thrAUC.pagerank_cent_neg.test_stat(con,ind,lev),out.thrAUC.pagerank_cent_neg.crit_val(con,ind,lev),out.thrAUC.pagerank_cent_neg.p_2tail(con,ind,lev),out.thrAUC.pagerank_cent_neg.hadNaN(con,ind),out.thrAUC.pagerank_cent_neg.hadimag(con,ind),out.thrAUC.pagerank_cent_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.pagerank_cent_pos_nodiscon.beta(con,ind,lev),out.thrAUC.pagerank_cent_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.pagerank_cent_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.pagerank_cent_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.pagerank_cent_pos_nodiscon.hadNaN(con,ind),out.thrAUC.pagerank_cent_pos_nodiscon.hadimag(con,ind),out.thrAUC.pagerank_cent_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.pagerank_cent_neg_nodiscon.beta(con,ind,lev),out.thrAUC.pagerank_cent_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.pagerank_cent_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.pagerank_cent_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.pagerank_cent_neg_nodiscon.hadNaN(con,ind),out.thrAUC.pagerank_cent_neg_nodiscon.hadimag(con,ind),out.thrAUC.pagerank_cent_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.part_coef == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.part_coef_pos.beta(con,ind,lev),out.full.part_coef_pos.test_stat(con,ind,lev),out.full.part_coef_pos.crit_val(con,ind,lev),out.full.part_coef_pos.p_2tail(con,ind,lev),out.full.part_coef_pos.hadNaN(con,ind),out.full.part_coef_pos.hadimag(con,ind),out.full.part_coef_pos.hadInf(con,ind));
            end
            ...
            if any(any(ind))
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.part_coef_neg.beta(con,ind,lev),out.full.part_coef_neg.test_stat(con,ind,lev),out.full.part_coef_neg.crit_val(con,ind,lev),out.full.part_coef_neg.p_2tail(con,ind,lev),out.full.part_coef_neg.hadNaN(con,ind),out.full.part_coef_neg.hadimag(con,ind),out.full.part_coef_neg.hadInf(con,ind));
            end
        end
        ...
        if out.test_props_thrmat.rich_club == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%u\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',ind,out.thrAUC.rich_club_pos.beta(con,ind,lev),out.thrAUC.rich_club_pos.test_stat(con,ind,lev),out.thrAUC.rich_club_pos.crit_val(con,ind,lev),out.thrAUC.rich_club_pos.p_2tail(con,ind,lev),out.thrAUC.rich_club_pos.hadNaN(con,ind),out.thrAUC.rich_club_pos.hadimag(con,ind),out.thrAUC.rich_club_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%u\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',ind,out.thrAUC.rich_club_neg.beta(con,ind,lev),out.thrAUC.rich_club_neg.test_stat(con,ind,lev),out.thrAUC.rich_club_neg.crit_val(con,ind,lev),out.thrAUC.rich_club_neg.p_2tail(con,ind,lev),out.thrAUC.rich_club_neg.hadNaN(con,ind),out.thrAUC.rich_club_neg.hadimag(con,ind),out.thrAUC.rich_club_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%u\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',ind,out.thrAUC.rich_club_pos_nodiscon.beta(con,ind,lev),out.thrAUC.rich_club_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.rich_club_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.rich_club_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.rich_club_pos_nodiscon.hadNaN(con,ind),out.thrAUC.rich_club_pos_nodiscon.hadimag(con,ind),out.thrAUC.rich_club_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%u\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',ind,out.thrAUC.rich_club_neg_nodiscon.beta(con,ind,lev),out.thrAUC.rich_club_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.rich_club_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.rich_club_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.rich_club_neg_nodiscon.hadNaN(con,ind),out.thrAUC.rich_club_neg_nodiscon.hadimag(con,ind),out.thrAUC.rich_club_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.strength == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.strength_pos.beta(con,ind,lev),out.full.strength_pos.test_stat(con,ind,lev),out.full.strength_pos.crit_val(con,ind,lev),out.full.strength_pos.p_2tail(con,ind,lev),out.full.strength_pos.hadNaN(con,ind),out.full.strength_pos.hadimag(con,ind),out.full.strength_pos.hadInf(con,ind));
            end
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.strength_neg.beta(con,ind,lev),out.full.strength_neg.test_stat(con,ind,lev),out.full.strength_neg.crit_val(con,ind,lev),out.full.strength_neg.p_2tail(con,ind,lev),out.full.strength_neg.hadNaN(con,ind),out.full.strength_neg.hadimag(con,ind),out.full.strength_neg.hadInf(con,ind));
            end
        ...
        if out.test_props_thrmat.sub_cent == 1
            ...
            if any(ind)
                ...
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.subgraph_cent_pos.beta(con,ind,lev),out.thrAUC.subgraph_cent_pos.test_stat(con,ind,lev),out.thrAUC.subgraph_cent_pos.crit_val(con,ind,lev),out.thrAUC.subgraph_cent_pos.p_2tail(con,ind,lev),out.thrAUC.subgraph_cent_pos.hadNaN(con,ind),out.thrAUC.subgraph_cent_pos.hadimag(con,ind),out.thrAUC.subgraph_cent_pos.hadInf(con,ind));
            end
            if out.calc_neg_mats == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.subgraph_cent_neg.beta(con,ind,lev),out.thrAUC.subgraph_cent_neg.test_stat(con,ind,lev),out.thrAUC.subgraph_cent_neg.crit_val(con,ind,lev),out.thrAUC.subgraph_cent_neg.p_2tail(con,ind,lev),out.thrAUC.subgraph_cent_neg.hadNaN(con,ind),out.thrAUC.subgraph_cent_neg.hadimag(con,ind),out.thrAUC.subgraph_cent_neg.hadInf(con,ind));
                end
            end
            if out.calcAUC_nodiscon == 1
                ...
                if any(ind)
                    ...
                    fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                    fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.subgraph_cent_pos_nodiscon.beta(con,ind,lev),out.thrAUC.subgraph_cent_pos_nodiscon.test_stat(con,ind,lev),out.thrAUC.subgraph_cent_pos_nodiscon.crit_val(con,ind,lev),out.thrAUC.subgraph_cent_pos_nodiscon.p_2tail(con,ind,lev),out.thrAUC.subgraph_cent_pos_nodiscon.hadNaN(con,ind),out.thrAUC.subgraph_cent_pos_nodiscon.hadimag(con,ind),out.thrAUC.subgraph_cent_pos_nodiscon.hadInf(con,ind));
                end
                if out.calc_neg_mats == 1
                    ...
                    if any(ind)
                        ...
                        fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                        fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.thrAUC.subgraph_cent_neg_nodiscon.beta(con,ind,lev),out.thrAUC.subgraph_cent_neg_nodiscon.test_stat(con,ind,lev),out.thrAUC.subgraph_cent_neg_nodiscon.crit_val(con,ind,lev),out.thrAUC.subgraph_cent_neg_nodiscon.p_2tail(con,ind,lev),out.thrAUC.subgraph_cent_neg_nodiscon.hadNaN(con,ind),out.thrAUC.subgraph_cent_neg_nodiscon.hadimag(con,ind),out.thrAUC.subgraph_cent_neg_nodiscon.hadInf(con,ind));
                    end
                end
            end
        end
        ...
        if out.test_props_fullmat.mod_deg_z == 1
            ind = logical(out.full.mod_deg_z.p_2tail(con,:,lev) <= out.alpha & ~isnan(out.full.mod_deg_z.beta(con,:,lev)) & out.full.mod_deg_z.beta(con,:,lev) ~= 0);
            if any(ind)
                out.sig_find(con).mod_deg_z_full{1,lev} = ['Contrast/F-test #',num2str(con)];
                out.sig_find(con).mod_deg_z_full{2,lev} = {'node','beta','stat','crit_val','p','had NaN','had imag','had Inf'};
                out.sig_find(con).mod_deg_z_full{2,lev} = [out.sig_find(con).mod_deg_z_full{2,lev};[out.ROI_labels(ind),num2cell([out.full.mod_deg_z.beta(con,ind,lev);out.full.mod_deg_z.test_stat(con,ind,lev);out.full.mod_deg_z.crit_val(con,ind,lev);out.full.mod_deg_z.p_2tail(con,ind,lev);out.full.mod_deg_z.hadNaN(con,ind);out.full.mod_deg_z.hadimag(con,ind);out.full.mod_deg_z.hadInf(con,ind)]')]];
                
                fprintf(sigeffects_fid,'Within-module degree z-score (full matrices):\n\n');
                fprintf(sigeffects_fid,'node\tbeta\tstat\tcrit_val\tp\thad NaN\thad imag\thad Inf\n');
                fprintf(sigeffects_fid,'%s\t%5.4f\t%5.4f\t%5.4f\t%5.4f\t%u\t%u\t%u\n\n',out.ROI_labels{ind},out.full.mod_deg_z.beta(con,ind,lev),out.full.mod_deg_z.test_stat(con,ind,lev),out.full.mod_deg_z.crit_val(con,ind,lev),out.full.mod_deg_z.p_2tail(con,ind,lev),out.full.mod_deg_z.hadNaN(con,ind),out.full.mod_deg_z.hadimag(con,ind),out.full.mod_deg_z.hadInf(con,ind));
            end
        end
    end
end
